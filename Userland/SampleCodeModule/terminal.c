...
const commandT commands[] = {
                            ...
                            {"memFree", "memory free", mem_free},
                            {"memUsed", "memory used", mem_used},
    ...
    for(int i = 0; i < SIZEOFARR(commands); i++){
        if(!strcmp(cmd, commands[i].name)) {
            ...
            if(strcmp(aux, "&") == 0){
                ...
                int j=0;
                while(strcmp(args[0], commands[j].name)!=0){
                    j++;
                }
                int pid = execve(getpid(), commands[j].function, arg_count, args, 1);
