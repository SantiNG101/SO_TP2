...
void keyboardHandler(){
    ...
    while((status = read_port(KEYBOARD_STATUS_PORT) & 0x01) == 0);

    key = read_port(KEYBOARD_DATA_PORT);
    
    if(isSpecial(key)){
        setState(keyboard[(key >> 4) & 0x07][key & 0x0F]);
        return;
    }
    ...
    uint8_t keyCode = keyboard[key >> 4][key & 0x0F];
    if(getState(SHF)){
        keyCode = shftKeyBoard[key >> 4][key & 0x0F];
    }
    putChar(keyCode);
}
